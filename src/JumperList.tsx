...
function JumperList() {
    ...
    function removeJumper(e: SyntheticEvent) {
        ...
        setJumpers(jumpers.filter(entry => {
            return entry.id !== id;
        }))

    }

    function handleKeyDown (e: KeyboardEvent) { // allow hitting enter to submit the new jumper
        if (e.key === 'Enter') {
    ...
    return (
        <div className='jumperListSection'>
            ...
            {jumpers.map(jumper => {
                ...
                return <Jumper key={jumper.id} jumper={jumper} removeJumper={removeJumper} filters={filters}/>
            })}
            </div>
